...
async function getLocInfoByIP(props: TInput): Promise<TOutput> {
  ...
  if (ip.length <= 0) {
    throw new Error("[Bad Req]: Get Loc Info By IP - Mandatory 'ip' field needs to have IPv4 or IPv6 address details");
  } else {
    /* setup */
    let API_URL = `${API_ROOT}/${ip}`;

    if (typeof fields === "string" && fields !== "default") {
      throw new Error("[Bad Req]: Get Loc Info By IP - Needs to be either 'default' or array[] of acceptable fields");
    } else {
